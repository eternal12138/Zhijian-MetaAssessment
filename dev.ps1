...
Push-Location $backendDir
try {
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase2.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-2 database migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase3.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-3 database migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase4.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-4 database migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_task_order.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the task-order assignment database migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase5.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-5 database migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase6.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-6 database migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase7.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-7 authoritative ASR migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase8.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-8 production-security migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase9.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-9 model-configuration migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase10.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-10 optional-questionnaire migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase11.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-11 questionnaire-version migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase12.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-12 blinded-coding migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase13.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-13 coding-scope migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase14.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-14 human-narration migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase15.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-15 task-wording migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase16.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-16 model-configuration-history migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase17.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-17 run-quality-review migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase18.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-18 questionnaire-participant-name migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase19.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-19 UTC timestamp migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase20.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-20 metacognitive-extraction migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase21.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-21 candidate-review lease migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase22.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-22 extraction-provenance migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase23.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-23 performance-index migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase24.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-24 report-notification migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase25.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-25 export-acceleration migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase26.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-26 model-training migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase27.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-27 observable model lifecycle migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase28.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to apply the phase-28 expert-dataset migration."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase29.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Phase 29 embedding provider migration failed with exit code $LASTEXITCODE."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase30.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Phase 30 model training progress migration failed with exit code $LASTEXITCODE."
    }
    & $venvPython (Join-Path $backendDir "scripts\migrate_phase31.py")
    if ($LASTEXITCODE -ne 0) {
        throw "Phase 31 prediction audit migration failed with exit code $LASTEXITCODE."
    }
