...
from scripts import asr_worker, export_worker, extraction_worker, model_training_worker


class WorkerEntrypointTests(unittest.TestCase):
    def test_all_background_workers_expose_runnable_entrypoints(self):
        for worker in (asr_worker, extraction_worker, export_worker, model_training_worker):
